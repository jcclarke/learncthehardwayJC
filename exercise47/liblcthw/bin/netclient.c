...
int nonblock(int fd)	
{
	...
	check(rc == 0, "Can't set nonblocking flag.");
...
int write_some(RingBuffer *buffer, int fd, int is_socket)
{
	...
	//RingBuffer_read(buffer, input, RingBuffer_available_data(buffer));
	//bstring data			= bfromstr(input);
...
int main(int argc, char* argv[])
{
	...
	while(1) {
		...
		while (!RingBuffer_empty(sock_rb)) {
			rc				= write_some(sock_rb, FD_STDOUT, 0);
