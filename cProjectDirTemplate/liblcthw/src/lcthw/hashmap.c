...
/*
 * Simple Bob Jenkins's hash algorithm taken
 * form the Wikipedia description.
 */
...
void *Hashmap_delete(Hashmap *map, void *key)
{
	...
	HashmapNode *node		= DArray_get(bucket, i);
	check(node != NULL, "Failed to get node from bucket during delete when it should exist.");
	void *data				= node->data;
	free(node);
	...
	if (ending != node) {
		// alright, it looks like it's not the last one. So swap it.
