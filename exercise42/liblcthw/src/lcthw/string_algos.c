		...
		) 
{
	...
	assert(haystack != NULL && "Given bad haystack to search for.");
...
StringScanner *StringScanner_create(
		bstring in
		)
{
	...
	assert(scan != NULL && "WTF!\nWhy the hell is scan = NULL?!");
	return scan;
...
static inline void StringScanner_set_needle(
		StringScanner *scan,
		bstring tofind
		)
{
...
static inline void StringScanner_reset(
		StringScanner *scan
		)
{
