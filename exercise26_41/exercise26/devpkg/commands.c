...
//#include "/usr/local/apr/include/apr-1/apr_uri.h"
//#include "/usr/local/apr/include/apr-1/apr_fnmatch.h"
#include "commands.h"
#include "dbg.h"
#include "bstrlib.h"
#include "db.h"
...
int Command_fetch(apr_pool_t * p, const char * url, int fetch_only)
{
	...
	if (apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		if (info.scheme) {
			...
			check(rc == APR_SUCCESS, "Failed to curl source: %s", url);
		}
		...
		if (info.scheme) {
			...
			check(rc == APR_SUCCESS, "Curl failed.");
		}
	...
	// Indicates an install needs to actually run
	return 1;
error:
	return -1;
}
