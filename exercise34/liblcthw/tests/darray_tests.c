...
char *test_push_pop()
{
	...
	printf("\n\nend\t\t%d", array->end);
	printf("\n\nmax\t\t%d", array->max);
	printf("\n\nmax\t\t%d", DArray_count(array));
	printf("\n\nelement_size\t%d", (int)array->element_size);
	printf("\n\nexpand_rate\t%d", (int)array->expand_rate);
	printf("\n\nDArray_first\t%d", DArray_first(array));
	printf("\n\nDArray_last\t%d\n\n", DArray_last(array));
	//mu_assert(array->end == 1000, "end isn't at the right spot. Should be 0 after DArray_create.");d;	int		max;	size_t	element_size;	size_t	expand_rate;
