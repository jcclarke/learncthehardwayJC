...
/*
 * A naive copy that assumes all inputs are always valid
...
{
	assert(from != NULL && to != NULL && "Neither from nor to can be NULL");

	int i		= 0;
	int max		= from_len > to_len - 1 ? to_len - 1 : from_len;

	// to_len and from_len must have atleast one byte
