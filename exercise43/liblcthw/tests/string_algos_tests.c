...
char *test_find_and_scan()
{
	...
	mu_assert(find_i > 0, "Failed to find 'ALPHA' in the string.");
	
	int scan_i					= StringScanner_scan(scan, &ALPHA);
	mu_assert(scan_i > 0, "Failed to find 'ALPHA' with scan.");
	mu_assert(scan_i == find_i, "find_i and scan_i don't match.");

	scan_i						= StringScanner_scan(scan, &ALPHA);
	mu_assert(scan_i > find_i, "Should find another 'ALPHA' after the first one.");
	int find_i_2				= scan_i;

	scan_i						= StringScanner_scan(scan, &ALPHA);
	mu_assert(scan_i > find_i_2, "Should find another 'ALPHA' after the second one.");

	mu_assert(StringScanner_scan(scan, &ALPHA) == -1, "Shouldn't find anymore 'ALPHA's.");
...
char * test_binstr_performance()
{
	...
	do {
		for (i = 0; i < 1000; i++){
			...
			mu_assert(found_at != BSTR_ERR, "Failed to find 'ALPHA'.");
			find_count++;
		}
...
char * test_find_performance()
{
	...
	do {
		for (i = 0; i < 1000; i++){
			...
			mu_assert(found_at > 0, "Failed to find 'ALPHA' in the string.");
			find_count++;
		}
...
// This is an idiom for commenting out code
//#if 0
